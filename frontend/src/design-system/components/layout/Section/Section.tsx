...
export function Section({
  ...
  return (
    <SectionDepthContext.Provider value={{ depth: nextDepth, baseLevel: effectiveBase }}>
      <Component className={classes} style={mergedStyle} data-depth={depth}>
        {renderHeader ? <div className="ds-section__header">{renderHeader}</div> : null}
        {subheader ? <div className="ds-section__subheader">{subheader}</div> : null}
        {children}
      </Component>
